  ...
  if (url.pathname.startsWith("/api/folders")) {
    ...
    try {
      ...
      if (request.method === "POST") {
        ...
        if (db) {
          await db.execute({
            sql: "INSERT OR REPLACE INTO folders (id, user_id, name, color, parent_id) VALUES (?, ?, ?, ?, ?)",
            args: [folderId, payload.userId, name.trim(), color || "", parent_id || null]
          });
        }
        return new Response(JSON.stringify({ success: true, data: { id: folderId, name: name.trim(), color, parent_id: parent_id || null } }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
    } catch (err: any) {
